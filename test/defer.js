...
(function(window, deferQueue) {
  ...
  */
  function destroyInstance(id) {
    window.setTimeout(function destroyInstanceImmediately() {
      _deferences[id] = null;
      delete _deferences[id];
    }, 0);
  };
  ...
  */
  function _deferInstance(thePredicate, theHandler, theOptions) {
    ...
    if (firstTest === YES) { // no need to defer
    ...
    if (timeout != undefined && !isNaN(timeout)) {
      ...
      if (self.timeoutMS > (+new Date())) {
        if (self.completed === NO) {
          // do it again
          self.timeoutID = window.setTimeout(function _deferInstance_setTimeout_impl_set() {
            self.resultHandler(self.test());
  ...
  */
  function handleReady(ev) { //log(ev);
    ...
    if (!isNil(ev) && !isNil(ev[kType])) {
      ...
      switch (ev[kType]) {
        ...
        case kLoad:
          returnValue = (ev.returnValue === YES);
          break;
  ...
  defer(function DOMReadyDeference() {
