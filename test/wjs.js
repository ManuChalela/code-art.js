(function(context) {
  ...
  WjsProto.prototype = {
    ...
    regEach: function(registry, callback) {
      var i = 0,
        j, type, name, types = Object.keys(registry),
        names;
      while (type = types[i++]) {
  ...
  WjsProto.registerListen = function(type, name, callback) {
    var self = this,
      data = WjsProto.retrieve(type, name);
    if (data) {
      callback(data);
    } else {
